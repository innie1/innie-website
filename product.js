...
if (!product) {
  root.innerHTML = `
    <a class="back" href="index.html">← Back to INNIE</a>
    <section class="detail-section" style="margin-top:0">
      <p class="eyebrow">PRODUCT / SERVICE</p>
  ...
  root.innerHTML = `
    ...
    <section class="detail-hero">
      ...
      <div>
        <p class="eyebrow">${escapeHtml(product.type === 'service' ? 'SERVICE' : 'PRODUCT')}</p>
        <h1>${escapeHtml(product.name)}</h1>
        <p class="detail-summary">${escapeHtml(product.shortDescription)}</p>
        <p class="detail-status"><span class="status-dot">${product.status === 'Live' ? '●' : '○'}</span> ${escapeHtml(product.status)}</p>
        ${product.appUrl && product.status === 'Live' ? `<a class="launch" href="${escapeHtml(product.appUrl)}" target="_blank" rel="noopener">Open ${escapeHtml(product.name)} →</a>` : ''}
      </div>
    </section>
    <section class="detail-section">
      <p class="eyebrow">ABOUT</p><h2>About this ${product.type === 'service' ? 'service' : 'product'}</h2><p>${escapeHtml(product.description)}</p>
    </section>
